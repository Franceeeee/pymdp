...
def update_posterior_policies(
    ...
    for idx, policy in enumerate(policies):
        ...
        if use_states_info_gain:
            G[idx] += calc_states_info_gain(A, qs_pi) * scale_state
...
#     return policies
def construct_policies(num_states, num_controls=None, policy_len=1, control_fac_idx=None, shared_control_groups=None):
    num_factors = len(num_states)
    ...
    for reduced_policy in reduced_policies:
        ...
        for t in range(policy_len):
            for group_idx, group in enumerate(shared_control_groups):
                value = reduced_policy[t * len(shared_control_groups) + group_idx]  # Valore della politica ridotta
                for factor in group:
                    expanded_policy[t, factor] = value  # Assegna lo stesso valore a tutti nel gruppo
        
        full_policies.append(expanded_policy)
